...
def process_accuracy_csv(args):
    ...
    k = ["schema_version","submit_date","component","subcomp","model","precision","mode","benchmark_infra","version","gpuarch","sdk_version","repo","commithash","total_count","pass_count","version_details","score_details","rocm_details","cuda_details"]
    compname = "pytorch"
    v = [
        "v1",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process Test reports csv files.")
    parser.add_argument('--subcomp', nargs=1, dest='subcomp', required=True, help="Subcomponent name")
    parser.add_argument('--run_id', nargs=1, dest='run_id', required=True, help="workflow run_id")
    parser.add_argument('--repo', nargs=1, dest='repo', required=True, help="github repo path")
